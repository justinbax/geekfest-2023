        ...
        <v-row>
            <v-col>
                <v-sheet rounded class="pa-6">
                    ...
                    <v-btn @click="cheeseCount++" prepend-icon="mdi-cheese">Cheese: {{ cheeseCount }}</v-btn>
                    <div></div>
                </v-sheet>
            </v-col>
        </v-row>
        <v-row>
            ...
            <v-col>
                <v-card title="Settings" class="pa-4">
                    <v-slider step="5" max="120" v-model="durationInMinutes" max-width="90%" :append="durationInMinutes">
                        <template v-slot:append>
                            {{ durationInMinutes }} minutes
                        </template>
                    </v-slider>
                    <v-textarea clearable label="Justification"></v-textarea>
                    <v-btn>Submit!</v-btn>
                </v-card>
            </v-col>
        </v-row>
...
<script setup lang="ts">
    ...
    let durationInMinutes: Ref<number> = ref(0)
